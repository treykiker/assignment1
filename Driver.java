...
public class Driver{
	...
	public static void main(String [] args){
		...
		for(int i = 0; i <= 59; i++){ // The for loop loops 60 times to simulate the 60 minutes in the store.
			...
			addCust = randNum.nextInt(4) + 1; // The value of 1-4 is given to the addCust.
			...
			if(maxLineSize == 0){ // If the maxLineSize is zero, the maximum size becomes whatever the queue size is at that iteration.
				...
				maxLineSize = cqueue.getSize(); // Ask prof if max line size should count iterations where a customer is added and removed.
												// When one is added max line size is 5 but if one is also removed then max line size becomes 4.
			}
